...
| `tests/test_002d_builder_migration.py` | image-builder CLI; CircleCI disabled; Cirrus retired |
| `tests/test_002e_github_actions.py` | GitHub Actions workflow contracts; public runner; 20% progress |

## CircleCI / GitHub Actions (Layer B)

| Command / artifact | Layer | Purpose |
|--------------------|-------|---------|
| CircleCI `m04-manual` | — | **Disabled** (`when: false`); do not retry |
| GitHub Actions `.github/workflows/m04.yml` | B | `workflow_dispatch` + `confirm_m04`; standard `ubuntu-24.04`; REVIEW ZIP only |
