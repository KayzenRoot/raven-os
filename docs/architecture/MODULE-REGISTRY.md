...
| M04 | Raven bootc base image + QCOW2 build path | 12 | `Containerfile`; `os/`; `scripts/build_*.py` | M01 | BLOCKED | CircleCI heavy disabled; Cirrus `.cirrus.yml` manual; Layer B not yet proven. |
